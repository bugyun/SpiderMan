...
class AutoClassVisitor extends ClassVisitor {
    ...
    void visit(int version, int access, String name, String signature, String superName, String[] interfaces) {
        ...
        Logger.info("\n||---开始扫描类：${mClassName}")
        Logger.
                info("||---类详情：version=${version};\taccess=${Logger.accCode2String(access)};\tname=${name};\tsignature=${signature};\tsuperName=${superName};\tinterfaces=${interfaces.toArrayString()}")
    ...
    MethodVisitor visitMethod(int access, String name, String desc, String signature, String[] exceptions) {
        ...
        if (GlobalConfig.isOpenLogTrack) {
            MethodVisitor logMethodVisitor = new LogMethodVisitor(methodVisitor, access, name, desc, mSuperName,
                    mClassName, mInterfaces, visitedFragMethods)
...
//            Logger.info("visitedFragMethods:" + visitedFragMethods)
            while (iterator.hasNext()) {
                ...
                LogAnalyticsUtil.
                        visitMethodWithLoadedParams(mv, Opcodes.INVOKESTATIC, LogHookConfig.LOG_ANALYTICS_BASE,
                                methodCell.agentName,
                                methodCell.agentDesc, methodCell.paramsStart, methodCell.paramsCount,
                                methodCell.opcodes)
